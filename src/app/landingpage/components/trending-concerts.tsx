  ...
  return (
    <section className="mx-auto w-full max-w-screen-lg">
      <div className="flex justify-between items-center mb-4">
        <h2 className="font-Nexa text-3xl font-bold text-[#1E3A8A]">Trending Concerts</h2>
          ...
          >
            {items.map((item) => (
              ...
              <div className="bg-[#FFF6F6] rounded-[40px] p-4 shadow-lg w-[270px] flex flex-col items-start text-left relative">
                ...
                <h3 className="font-Roboto text-[#1E3A8A] text-lg font-bold mt-3">{item.title}</h3>
                <p className="font-Roboto text-sm text-gray-600">{item.venue}</p>
                <p className="font-Roboto text-[11px] text-gray-600">{item.date} | {item.time}</p>
              </div>
            </div>            
            ))}
          </div>
